...
- \`blocker\`: the divergence will cause an actual bug, test failure, or CI failure on merge — raw \`prisma.model\` injection where the repo enforces \`createPrismaBase\` AND the diff depends on functionality the base class provides; missing \`@ResponseSchema\` / guard decorators that cause real misbehavior (unvalidated response goes to a typed consumer, an auth guard is skipped on a privileged route, the global ZodResponseInterceptor silently breaks); repo-enforced lint-banned pattern (for example \`any\` where configured, raw \`Date\` math where configured, unused imports) on a path where \`npm run verify\` would fail on merge; string / number literal in place of a library-provided enum where the upstream value silently changing would break the code.
