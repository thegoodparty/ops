...
- **GPT-5.6 Sol on Bedrock.** A child process (`delegate/review/`) runs the *same* scout and deep-reviewer prompts against `us.openai.gpt-5.6-sol`, concurrently with the Claude pass. It is SigV4-signed as the ECS task role — there is no static API key — and the model must be subscribed in the account out of band, because the task role deliberately has no marketplace permission (see `scripts/enable-bedrock-models.ts`). It runs with **no shell and no GitHub token**: its only tools are `read_file`, `list_files`, `grep` and `read_diff`, all read-only and all confined to the PR checkout, and it reads the diff the worker captured at `$PR_DIFF_FILE` rather than the API. It posts nothing itself. The process is driving a model over PR content nobody has vetted, and it shares the worker's UID, so the env allowlist it also runs under is defense in depth — withholding execution is what makes the confinement a boundary.
